...
var userInfoWriteQuery = 'UPDATE appuser SET sex=?, age=?, weight=?, height=?, targetweight=?, targetperiod=?, workperiod=?, worklevel=? WHERE id=?'; // 유저 정보입력 query, 이 외의 추가적인 정보 더 입력
            ...
            connection.query(userIdSearchQuery, [user.id], function(err, rows) {
                if (!err) { // query가 오는 경우
                    ...
                    if(!rows[0]) { // 중복 id 존재x
                        ...
                        connection.query(userInfoWriteQuery, [user.sex, user.age, user.weight, user.height, user.targetweight, user.targetperiod, user.workperiod, user.worklevel, user.id], function(err) {
                            if (err) {
                                console.log(err)
                                resultCode = dbResult.Fail;
                                connection.release();
